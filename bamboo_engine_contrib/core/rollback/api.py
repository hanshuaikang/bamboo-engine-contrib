...
from bamboo_engine_contrib.core.exceptions import RollBackException
...
def compute_validate_nodes(node_id, nodes, node_map):
    ...
    for target in targets:
        return compute_validate_nodes(target, nodes, node_map)
...
def clean_engine_data(runtime, pipeline_id, target_state):
    ...
    # 清理状态信息
    State.objects.filter(root_id=pipeline_id, node_id__in=need_clean_node_id_list).delete()
    # 清理Schedule 信息
